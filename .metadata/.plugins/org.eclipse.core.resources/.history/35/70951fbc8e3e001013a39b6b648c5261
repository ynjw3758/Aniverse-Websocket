package com.Anivers.Patform.Websocket.Controller;
...
public class Message_Controller {
    	...
    	if(resultmsg.get("result").equals(true)) {
            ...
            if(message.getFirst() == false) {
            	...
            	if (message.getInviteIds() != null && !message.getInviteIds().isEmpty()) {
            		for(String toUser : message.getInviteIds()) {
                    	String SessionId = Redis.getUserSession(toUser);
                    	Map<String, Object> send_msg = new HashMap<>();
                    	send_msg.put("ChatId", message.getChatId());
                    	send_msg.put("SendId", message.getSendId());
                    	send_msg.put("SendProfile", message.getProfile());
                    	...
                    	send_msg.put("SendMsg", message.getMessage());
                    	send_msg.put("SendTime", message.getTimestamp());
                    	JSONObject json = new JSONObject(send_msg);
                    	if (SessionId != null) {
                    		for(String UserIds : message.getInviteIds()) {
                        	    // userId를 기준으로 /user/{userId}/queue/notify 에 메시지 전송
                        	    messagingTemplate.convertAndSendToUser(
                        	    		UserIds,           // userId (principal name)
                        	        "/queue/notify",               // 대상 경로 (프론트는 여기를 구독해야 함)
                        	        send_msg                    // 보낼 데이터
                        	    );
                    		}
                    	    ...
                    	    logger.warn("❌ 세션 ID를 찾을 수 없습니다: {}", message.getSendId());
                            Map<String, Object> errorMsg = new HashMap<>();
                            errorMsg.put("code",1005);
                            errorMsg.put("reason", "보낸 사람 아이디를 확인할수 없음");
            		...
            		logger.error("초대 맴버의 데이터가 존재하지 않습니다");
                    Map<String, Object> errorMsg = new HashMap<>();
                    errorMsg.put("code",1005);
                    errorMsg.put("reason", "초대 맴버의 데이터가 존재하지 않습니다");
                    errorMsg.put("timestamp", LocalDateTime.now().toString());
            		...
            		logger.error("전송된 데이터가 비어있는 에러 발생" + e);
                    Map<String, Object> errorMsg = new HashMap<>();
                    errorMsg.put("code",1005);
                    errorMsg.put("reason", "전송된 데이터가 비어있는 에러 발생");
                    errorMsg.put("timestamp", LocalDateTime.now().toString());
    ...
    private MessageHeaders createJsonHeader() {
