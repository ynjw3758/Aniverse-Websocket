...
@Service("query_Mongo")
public class Query_Mongo {
	
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	private final MongoTemplate mongo_template;
	
	public Query_Mongo(MongoTemplate mongo_template) {
	...
	public Map<String, Object>UnreadChat(Map<String, Object> info){
		...
		if (lastread != null) {
		    ...
		    // lastread가 없을 경우 처리 (예: 전체 조회, 기본 시간 설정 등)
		    logger.info("📭 LastChatRead 조회 결과 없음, 전체 메시지 조회로 대체 : " + info);
		    Query unreadQuery = new Query();
